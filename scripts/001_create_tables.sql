...
-- RLS Policies for notices (anyone can read, only authors/admins can modify)
...
create policy "tags_insert_admin" on public.tags for insert with check (auth.uid() is not null);
