...
import { ArrowLeft, Heart, Share2, Bookmark, Check, ChevronDown, X } from "lucide-react"
...
export default function NoticePage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      <article className="bg-white dark:bg-neutral-800 rounded-lg shadow p-8 mb-8">
        <div className="flex items-start justify-between mb-6">
          <div>
            {notice.is_important && (
              <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded mb-2 inline-block">Important</span>
            )}
            {notice.is_featured && (
        ...
        <Link href={`/profile/${author?.id}`}>
          <div className="flex items-center gap-4 mb-6 pb-6 border-b hover:opacity-70 transition-opacity cursor-pointer">
            {author?.profile_image_url ? (
              ...
              <div className="w-12 h-12 rounded-full bg-neutral-300 dark:bg-neutral-600 flex items-center justify-center text-white font-semibold">
                {author?.display_name?.charAt(0)?.toUpperCase() || "?"}
              </div>
            )}
            <div>
              <p className="font-medium">{author?.display_name || "Anonymous"}</p>
              <p className="text-sm text-muted-foreground">{new Date(notice.created_at).toLocaleDateString()}</p>
            </div>
          </div>
        </Link>

        <p className="text-lg mb-8 leading-relaxed whitespace-pre-wrap">{notice.description}</p>
                ...
                <div className="space-y-2">
                  {files.map((file) => (
                    <a
                      ...
                      className="flex items-center gap-3 p-3 border rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors"
                    >
        ...
        <div className="flex gap-4">
          <Button variant={hasReacted ? "default" : "outline"} onClick={handleReact} disabled={!user}>
            <Heart size={16} className="mr-2" fill={hasReacted ? "currentColor" : "none"} />
            Like ({reactionCount})
          </Button>
      ...
      {user ? (
        <div className="bg-white dark:bg-neutral-800 rounded-lg shadow p-8 mb-8">
          ...
          <form onSubmit={handleAddComment} className="mb-6">
            ...
            <Button type="submit" disabled={!commentText.trim() || isPosting}>
              {isPosting ? "Posting..." : "Post Comment"}
            </Button>
          </form>
        </div>
      ) : (
        <div className="bg-white dark:bg-neutral-800 rounded-lg shadow p-8 mb-8 text-center">
                ...
                {comment.replies && comment.replies.length > 0 && (
                  <div className="ml-6 border-l-2 border-neutral-200 dark:border-neutral-700 pl-4">
                    <button
                      ...
                      className="flex items-center gap-2 py-2 text-sm font-medium text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-200 transition-colors"
                    >
                      <ChevronDown
                        size={16}
                    ...
                    {expandedReplies.has(comment.id) && (
                      <div className="space-y-3 mt-3">
                        {comment.replies.map((reply) => (
                          <div key={reply.id} className="bg-neutral-50 dark:bg-neutral-900 rounded p-4 space-y-3">
                            <div className="flex items-start gap-3">
                              {reply.profiles?.profile_image_url ? (
                                ...
                                <div className="w-8 h-8 rounded-full bg-neutral-300 dark:bg-neutral-600 flex items-center justify-center text-xs font-semibold flex-shrink-0">
                                  {reply.profiles?.display_name?.charAt(0)?.toUpperCase() || "?"}
                                </div>
                              )}
                              <div className="flex-1 min-w-0">
                                <p className="font-medium text-sm">{reply.profiles?.display_name}</p>
                                <p className="text-xs text-muted-foreground">
                                  {new Date(reply.created_at).toLocaleDateString()}
                                </p>
                              </div>
                            </div>
                            <p className="text-sm whitespace-pre-wrap text-neutral-700 dark:text-neutral-300">{reply.content}</p>
